...
contract AnswerToEarn is ReentrancyGuard, Ownable {
  ...
  struct QuestionStruct {
    ...
    uint created;
    uint answers;
    string tags; // We could use an array for storing the tags, but for the sake of simplicity, we use string
    uint256 prize;
  }
  ...
  mapping(uint => mapping(uint => AnswerStruct)) answersOf; // maps a question to its answers
  ...
  function createQuestion(string memory title, string memory description, string memory tags) public payable {
    ...
    question.tags = tags;
    question.prize = msg.value; // When the user/owner of the question creates a question, they have to pay the contract so the contract can pay the winner 
  ...
  function updateQuestion(uint qid, string memory title, string memory description, string memory tags) public {
