...
const toWei = (num) => ethers.utils.parseEther(num.toString()) // string => bigNumber
const fromWei = (num) => ethers.utils.formatEther(num) // wei => string

describe('Contracts', () => {

  const serviceFee = 5
  const id = 1 // The id of the question
  const aid = 1
  ...
  describe('Question Management', () => {
    ...
    it('Should verify update of title, description, and tags', async () => {
      ...
      await expectRevert(contract.connect(commentor1).updateQuestion(id, updatedTitle, description, tags),'Unauthorized entity!')
      await expectRevert(contract.updateQuestion(100, updatedTitle, description, tags),'Question not found')
      ...
      await expectRevert(contract.updateQuestion(id, updatedTitle, description, tags),'Question already with answer(s)')
    })
  ...
  describe('Paying winners', () => {
    ...
    beforeEach(async () => {
      ...
      await contract.createQuestion(title, description, tags, {value: toWei(prize),})
      await contract.connect(commentor1).addAnswer(id, 'Comment 1')
      await contract.connect(commentor2).addAnswer(id, 'Comment 2')
    })
  ...
  describe('Change Fee', () => {
    ...
    it('Should verify that the owner can change the service fee successfully', async () => {
      ...
      await expectRevert(contract.connect(commentor1).changeFee(serviceFee + 4),'Ownable: caller is not the owner')
    })
  })
})
